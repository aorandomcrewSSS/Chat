...
Цель — поддерживать чистоту и экономить пространство, не допуская накопления избыточных логов.
...
- **catchup = false** — пропущенные запуски не восполняются,
- настроена повторная попытка при ошибках: **до 2 раз**, с интервалом **1 минута**.
...
| любой другой       | PROD  | 30 дней       |

---

## Принцип работы

DAG состоит из трёх последовательных шагов.
...
- используется исключительно для построения последовательности задач.
...
DAG запускается каждые 3 дня, начиная с 29.07.2025 11:00 UTC.
